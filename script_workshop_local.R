...
# This script is the local import script for the PA AFS Workshop "Geospatial Analysis in R"
...
crs_wgs84 <- readRDS('workshop_files/crs_wgs84.rds')
crs_wgs84
...
geology_sf <- readRDS('workshop_files/geology_sf.rds') %>% 
...
nhd_sf <- readRDS('workshop_files/nhd_sf.rds') %>% 
  st_transform(crs_wgs84)
nhd_sf
st_crs(geology_sf) # now in WGS 84
...
roads_sf <- readRDS('workshop_files/roads_sf.rds') %>% 
  st_transform(crs_wgs84) 
roads_sf
